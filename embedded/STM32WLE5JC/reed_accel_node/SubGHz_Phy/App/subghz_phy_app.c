...
volatile bool wake_from_reed = false;
...
static uint8_t last_reed_state = 0xFF;
static uint32_t last_reed_time = 0;
/* USER CODE END PV */
...
void Master_Radio_Send(void) {
	...
	uint8_t current_reed = HAL_GPIO_ReadPin(REED_GPIO_Port, REED_Pin);

	uint8_t VAL_CLOSED = 0;    // magnet
	uint8_t VAL_OPEN = 1;      // no magnet
	uint8_t VAL_VIBRATION = 2; // vibrations

	bool is_door_closed = (current_reed == GPIO_PIN_RESET);
	...
	// reed switch changed state
	if (is_reed) {
		uint32_t now = HAL_GetTick();
		if (now - last_reed_time > 500) {
